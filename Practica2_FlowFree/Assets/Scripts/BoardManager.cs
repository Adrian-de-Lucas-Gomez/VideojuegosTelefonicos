    ...
    public class BoardManager : MonoBehaviour
    {
        ...
        private void HandleInput()
        {
            ...
            if (IsPosInBoard(worldPos))
            {
                ...
                else if (Input.GetMouseButton(0) && isBuildingFlow) //Si el usuario est� pulsando el bot�n izquierdo
                {
                    if(newTile != currentTile && !flows[currentFlow].isClosed()) //Nos hemos movido de casilla
                    {
                        ...
                        if(dir != Direction.None) //Si el movimiento ha sido v�lido (El movimiento en diagonal es demasiado poderoso)
                        {
                            if (tiles[newTile].IsActive()) //La casilla forma parte de un flujo
                            {
                                if (newFlow != currentFlow) //Nos hemos cruzado con otro flujo
                                {
                                    ...
                                    {
                                        //Cortar el otro flujo y avanzar en esa direcci�n
                                        currentTile = newTile;
                                    }
                                    else
                                    {
